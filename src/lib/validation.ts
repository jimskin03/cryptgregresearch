...
export const ProjectStatusSchema = z.enum(['LIVE', 'EXPERIMENTAL', 'IN DEVELOPMENT', 'CONCEPT', 'ARCHIVED', 'OFFLINE']);
...
export const DatasetSchema = z.object({
  ...
  status: z.enum(['VERIFIED', 'EXPERIMENTAL', 'PENDING']),
});
...
export const EvidenceSchema = z.object({
  id: z.string(),
  type: z.enum(['experiment-run', 'transaction', 'deployment', 'dataset', 'screenshot', 'commit', 'benchmark', 'simulation-state', 'agent-log']),
  status: EvidenceStatusSchema,
  project: z.string(),
  created_at: z.string().datetime(),
  sources: z.array(z.string().startsWith('/')),
});
...
export const TreasurySchema = z.object({
  ...
  address: z.string(),
  balance: z.string(),
  assets: z.array(z.object({ symbol: z.string(), amount: z.string(), usd: z.string() })),
